...
const Notifications = () => {
    ...
    useEffect(() => {
        const getAllFaqs = async () => {
            setIsLoading(true);
            setNotificationdata([]);
            const allNotifications = await authFetcher(
                getEndpointUrl(ENDPOINTS.getAllAdminNotifications),
            );
    ...
    const deleteNotification = async () => {
        await deleteItem(`${getEndpointUrl(ENDPOINTS.deleteAdminNotification(notificationId))}`)
            .then((result) => {
                if (result) {
                    setLoad(true);
    ...
    const notificationHide = async () => {
        await authFetcher(getEndpointUrl(ENDPOINTS.notificationHideAndShow(notificationId)))
            .then((result) => {
                if (result) {
                    setLoad(true);
    ...
    const tagsRemove = (content: string) => {
                ...
                <div className="faq_content col-span-4">
                    <div className="sm:flex sm:items-center sm:justify-between">
                        <div className="sm:text-left">
                            <h1 className="font-bold text-gray-900 header-font"> Notifications </h1>
                        </div>
                    </div>
                    <div className="text-end my-6">
                        <button type="button" className="py-2.5 px-5 me-2 text-sm font-medium  focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-blue-300 hover:text-white focus:z-10 focus:ring-4 focus:ring-gray-100 button_bg_2"><Link href="/admin/notifications/0" className="">Add Notification</Link></button>
                    </div>
                    {!isLoading ?
                        <div className="space-y-4 pb-6">
                            {notificationData && notificationData.length > 0 &&
                                <>
                                    {notificationData.map((notifications: notificationTypes, index: number) => {
                                        return (
                                            <div className="space-y-6">
                                                <div className="border_list">
                                                    <div className="text-base font-medium lg:flex lg:items-center">
                                                        <div className="lg:flex">
                                                            <div>{formatDate(notifications.createdAt) + " - "}</div>
                                                            <div className="leading-relaxed text-gray-700 lg:pl-1 truncate lg:w-[760px]">{tagsRemove(notifications.notificationDescription)}</div>
                                                        </div>
                                                        <div className="ms-auto text-end lg:mt-0 mt-3">
                                                            <div className="w-[150px] text-end">
                                                                <button type="button" className="py-1.5 px-1.5 me-2 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg  hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-100" onClick={() => { setDisplayModel(true); setNotificationId(notifications.notificationId); setIsHide(notifications.isDisplay) }}>
                                                                    {notifications.isDisplay ?
                                                                ...
                                                                <button type="button" className="py-1.5 px-1.5 me-2 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg  hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-100"><Link href={`notifications/${notifications.notificationId}`}>
                                                                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 32 32" fill="none">
                                                                ...
                                                                <button type="button" className="py-1.5 px-1.5 me-2 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg  hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-100" onClick={() => { setDeleteModel(true); setNotificationId(notifications.notificationId); }}>
                                                                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 32 32" fill="none">
                                                    ...
                                                    {notifications.startDate != null && notifications.endDate != null && <div className="text-center italic text-sm pt-2">This post is pinned to the top of the notification panel between {formatDate(new Date(notifications.startDate))} - {formatDate(new Date(notifications.endDate))}</div>}
                                                </div>
                                            </div>
                                        )
                                    })}
            ...
            >
                ...
                <Modal.Body>
                    <div className="space-y-6">
                        <div className="">
                            <p className="text-sm default_text_color font-normal leading-6">
                                {isHide ? 'You are about to hide the Notification' : 'You are about to show the Notification'}
                            </p>
                        </div>
                    </div>
                </Modal.Body>
