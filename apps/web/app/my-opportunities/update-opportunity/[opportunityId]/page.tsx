...
const MyOpportunitynotanonymous = () => {
  ...
  const { opportunityId } = paramsdata;
  const {
    register,
    handleSubmit,
    watch,
  ...
  const watchshowCompanyNameBox = watch("showCompanyNameBox", false);
  const watchShowPersonName = watch("showContactPerson", true);
  const watchServiceSelected = watch("servicesselected", []);
  const watchInputfiles = watch("inputfiles", []);
  const [platformsmasterdata, setPlatformsMasterData] = useState<
    platformsDto[]
  >([]);
  ...
  const breadcrumbItems = [
    ...
    {
      label: PATH.UPDATEOPPERTUNITY.name,
      path: PATH.UPDATEOPPERTUNITY.path,
    },
  ];
  ...
  useEffect(() => {
    ...
    async function getplatforms() {
      try {
        ...
        if (serviceslist) {
          setPlatformsMasterData(serviceslist);
    ...
    async function getOpportunityDetails() {
      const oppoResponse = await fetcher(
        getEndpointUrl(ENDPOINTS.getAllopportunityDetails(Number(opportunityId), Number(user?.companyId))),
      );
      
      if (oppoResponse.success) {
        // setFormDetails(oppoResponse.list);
        if(!oppoResponse.list){
          router.replace('/unauthorized')
        }
        if (user) {
          const result2 = await authFetcher(
            `${getEndpointUrl(
              ENDPOINTS.getOpportunityServices(Number(opportunityId)),
            )}`,
          );

          const capabilityIds: string[] = [];
          for (const capability of result2.data.capabilities) {
            capabilityIds.push(capability.capabilityId.toString());
          }
          setSelectedCapabilities(capabilityIds);
          const serviceIds: string[] = [];
          for (const serviceslist of result2.data.services) {
            serviceIds.push(serviceslist.serviceId.toString());
          }
          setSelectedServices(serviceIds);

          setValue("servicesselected", serviceIds as never[]);
          setValue("capabilitiesSelected", capabilityIds as never[]);
        }

        const upladedurls: { type: string; fileUrl: string }[] = [];
        const upladedDbUrls: { type: string; fileUrl: string }[] = [];
        if (oppoResponse.list.FileUploads.length > 0) {
          setImageCount(0)
          setPdfCount(0)
          oppoResponse.list.FileUploads.map(
            (files: { fileUrl: string; dburl: string, type: string }) => {
              if (files.type == 'file') {
                setPdfCount((prevPdfcount) => prevPdfcount + 1);
              } else {
                setImageCount((prevImageCount) => prevImageCount + 1)
              }
              upladedurls.push({ fileUrl: files.fileUrl, type: files.type });
              upladedDbUrls.push({ fileUrl: files.dburl, type: files.type });
            },
          );
          setUploadedFileUrls(upladedDbUrls);
          setUploadedSignedFileUrls(upladedurls);
        }
        if (oppoResponse.list.PlatformsOpt.length > 0) {
          const updatedplatforms: platformsDto[] = [];
          const updatedplatformsDB: number[] = [];
          oppoResponse.list.PlatformsOpt.map(
            (platform: { platforms: { id: number; name: string } }) => {
              const numberValue = platform.platforms.id;
              updatedplatformsDB.push(platform.platforms.id);
              updatedplatforms.push({
                id: numberValue,
                name: platform.platforms.name,
              });
            },
          );
          setSelectedPlatforms(updatedplatforms);
          setValue("platforms", updatedplatformsDB as never[]);
        }

        if (oppoResponse.list.approxStartDate) {
          setStartDate(new Date(oppoResponse.list.approxStartDate));
        }
        if (oppoResponse.list.approxEndDate) {
          setEndDate(new Date(oppoResponse.list.approxEndDate));
        }
        if (oppoResponse.list.expiryDate) {
          setOppExpiryDate(new Date(oppoResponse.list.expiryDate));
        }
        setValue("industryId", oppoResponse.list.industryId);
        setValue("oppStatus", oppoResponse.list.oppStatus);
        setValue("name", oppoResponse.list.name);
        setValue("description", isValidJSON(oppoResponse.list.description) ? JSON.parse(oppoResponse.list.description) : oppoResponse.list.description);
        setValue(
          "approxEndDateCondition",
          oppoResponse.list.approxEndDateCondition.toString(),
        );
        setValue(
          "approxStartDateCondition",
          oppoResponse.list.approxStartDateCondition.toString(),
        );
        setValue("showCompanyNameBox", oppoResponse.list.showCompanyName);
        // setValue("showContactPerson", oppoResponse.list.showContactPerson);
        if (oppoResponse.list.approxStartDate) {
          setValue(
            "approxStartDate",
            new Date(oppoResponse.list.approxStartDate).toLocaleDateString(
              "en-US",
            ),
          );
        }
        ...
        if (oppoResponse.list.expiryDate) {
          setValue(
            "expiryDate",
            new Date(oppoResponse.list.expiryDate).toLocaleDateString("en-US"),
          );
        }
        if (oppoResponse.list.approxEndDate) {
          setValue(
            "approxEndDate",
            new Date(oppoResponse.list.approxEndDate).toLocaleDateString(
              "en-US",
            ),
          );
        }
        setValue("technologies", oppoResponse.list.technologies);
        setValue("staffMonths", oppoResponse.list.staffMonths);
        user?.companies[0].name &&
          setValue("showcompanyname", user?.companies[0].name);
        if (oppoResponse.list.contactPersonName) {
          setValue("contactPersonName", oppoResponse.list.contactPersonName);
        }

        setValue(
          "isReceiveEmailEnabled",
          oppoResponse.list.isReceiveEmailEnabled,
        );
      } else {
        console.log(`Api responded with statuscode ${oppoResponse.statusCode}`);
      }
      setCanRender(true);
    }
    getservices();
  ...
  async function handleUploadedFile(event: React.FormEvent<HTMLInputElement>) {
    ...
    const fileSignedUrls: { type: string; fileUrl: string }[] = [
        ...
        .then((result) => {
          if (result.data.data) {
            ...
            savedforDBFileUrls.forEach((url) => {
              ...
              fileUrls.push({ type, fileUrl: url.url });
            });
  ...
  const handleFileRemove = (index: number) => {
    ...
    if (deletedfile && deletedfile[0].fileUrl) {
      if (deletedfile[0].type == 'file') {
        ...
        setImageCount((prevImageCount) => prevImageCount - 1);
        const newFilePath = [...deletedFilePaths,deletedfile[0].fileUrl];
  ...
  //save form
  const { error, success, submitForm } = CommonPatchData<opportunityForm>({
    url: getEndpointUrl(ENDPOINTS.updatemyOpportunity),
  });
  ...
  const onSubmit = (data: opportunityForm) => {
    data.servicesselected = selectedServices;
    data.capabilitiesSelected = selectedCapabilities;
    if (pdfCount > 1) {
      setError('inputfiles', {
        type: 'manual',
        message: 'You can only upload one PDF file'
    ...
    if (data.approxEndDateCondition == "3") {
      data.approxEndDate = new Date(data.approxEndDate).toLocaleDateString(
        "en-US",
      );
    } else {
      data.approxEndDate = "";
    }
    if (data.approxStartDateCondition == "3") {
      data.approxStartDate = new Date(data.approxStartDate).toLocaleDateString(
        "en-US",
      );
    } else {
      data.approxStartDate = "";
    }
    data.id = Number(opportunityId);
    ...
    const  sanitizedData = sanitizeData(data) as opportunityForm;
    sanitizedData.description = JSON.stringify(sanitizedData.description);
    sanitizedData.uniqueFormId = uniqueFormId;
    sanitizedData.deletdFilePaths = deletedFilePaths;
    submitForm(sanitizedData)
      .then((response) => {
        if (response.data && response.data.StatusCode === 200) {
          setButtonLoader(false);
          toast.success('Opportunity successfully updated 👍');
          const timeoutId = setTimeout(() => {
            router.push("/my-opportunities");
          }, 1000);
          return () => clearTimeout(timeoutId);
  ...
  function onAddOrRemovePlatforms(
    ...
    if (platforms.length > 0) {
      if (errors.platforms && errors.platforms) {
          ...
          <form onSubmit={handleSubmit(onSubmit)}>
            <div className="flex justify-between">
              <div className="text-left">
                <h1 className="defult_text_color header-font">
                  Update Opportunity
                </h1>
              </div>
            </div>
              ...
              <div className="grid lg:grid-cols-3 lg:gap-28 gap-10">
                <div className="pe-4">
                  ...
                  <div className="companyName flex items-center gap-2">
                      <Checkbox
                        id="companyName"
                        {...register("showCompanyNameBox")}
                      />
                      <Label htmlFor="showCompanyNameBox" className="flex">
                        Show Company Name
                      </Label>
                      {/* <Tooltip
                    ...
                    <div className="files">
                      ...
                      >
                        <div className="flex flex-col items-center justify-center pb-6 pt-5">
                          ...
                          <p className="text-xs text-gray-500 dark:text-gray-400">PNG, JPG, JPEG, PDF (Max 10 images & 1 PDF)</p>
                      ...
                      <div className="grid grid-cols-2 gap-2 pt-6">
                        {uploadedSignedFileUrls &&
                          uploadedSignedFileUrls.map((url, index) => (
                            <div
                              className="relative img_view_h"
                              key={`fileurl_${index}`}
                            >
                              <hr />
                              {/* <Link href={url.fileUrl} target="__blank">{url.fileUrl} </Link> */}
                              {(url.type == 'image') ? <>
                                ...
                                <Link prefetch={false} href={url.fileUrl} target="__blank" className="cursor-pointer">
                              ...
                              {/* <Image
                              src="/cross.svg"
                              className="w-3 h-3 inline-flex align-top	 ms-1 cross_icon divide-y "
                              alt=""
                              width={15}
                              height={15}
                ...
                <div className="pe-4">
                  <div className="flex max-w-md flex-col gap-5">
                    ...
                    <div className="status">
                      <div className="mb-2 block">
                        <Label htmlFor="industryId" className="font-bold text-xs">
                      ...
                      >
                        <option value={""}>Select</option>
                        {industryTypes &&
                          industryTypes.map((industries, index) => (
                            <option
                              key={`${industries.name}_${index}`}
                              value={industries.id}
                            >
                              {industries.name}
                            </option>
                      ...
                      <div className="status">
                        <div className="mb-2 block">
                            <Label htmlFor="Platforms" className="font-bold text-xs">
                    ...
                    <div className="status">
                      <div className="mb-2 block">
                        <Label htmlFor="base" className="font-bold text-xs">
                      ...
                      <fieldset className="flex max-w-md flex-col gap-2">
                        <div className="flex items-center gap-2">
                          <Radio
                            ...
                            value={1}
                          />
                          <Label htmlFor="startdatecondtion">
                            To Be Determined
                          </Label>
                        </div>
                        <div className="flex items-center gap-2">
                        ...
                        {/* <p className="text-red-600 text-xs">
                          {typeof errors.approxStartDateCondition?.message ===
                            "string" && errors?.approxStartDateCondition.message}
                        ...
                        <div className="pl-6 pt-4">
                          ...
                          <DatePicker
                            ...
                            {...register("approxStartDate", {
                              required: {
                                value: true,
                                message: "Approximate Start is required",
                              },
                            })}
                    ...
                    <div className="status">
                      ...
                      <fieldset className="flex max-w-md flex-col gap-2">
                        ...
                        {/* <p className="text-red-600 text-xs">
                          {typeof errors.approxEndDateCondition?.message ===
                            "string" && errors?.approxEndDateCondition.message}
                        </p> */}
                      </fieldset>
                      {watchEndDate && watchEndDate == "3" && (
                        <div className="pl-6 pt-4">
                          {/* <TextInput
                            ...
                            {...register("enddate", {
                              validate: (value) => {
                                if (watchEndDate == "3" && !value) {
                                  return "Specific end date is Required";
                                }
                                return true;
                              },
                            })}
                          ...
                          <DatePicker
                            ...
                            {...register("approxEndDate", {
                              required: {
                                value: true,
                                message: "Approximate End is required",
                              },
                            })}
                            onChange={(date: Date) => {
                    ...
                    <div className="Estimated">
                      <div className="mb-2 block">
                        <Label htmlFor="base" className="font-bold text-xs">
                        Estimated Number of Staff Months{" "}
                          <span style={{ color: "red" }}>*</span>
                        </Label>
                      </div>
                      <TextInput
                ...
                <div className="third_section pl-6">
                  <h3 className="text-sm font-bold pb-3">
                  ...
                  >
                            ...
                            </div>
                            {
                              <>
                                {selectedServices.includes(
                                  ...
                                  <ul className="ps-8 pt-3 space-y-3 text-sm text-gray-700 dark:text-gray-200" key={`mobileservices${service.serviceName + cIndex}`}>
                                    <li key={cIndex}>
                                      <div className="flex items-center">
                                        <input
                    ...
                    <p className="text-red-600 text-xs pt-4">
                      {watchServiceSelected && errors.servicesselected?.message}
                    </p>
                  </div>
                </div>
              </div>
