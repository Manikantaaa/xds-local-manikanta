...
import SharingExceedReport from "./sharingexeedreports";

const ExportExceedReport = () => {
  ...
  const breadcrumbItems = [
    ...
    {
      label: "Companies Export List",
      path: "Companies Export List",
    },
  ];
  const [canRender, setCanRender] = useState(false);

  useEffect(() => {
    authFetcher(`${getEndpointUrl(ENDPOINTS.getExportReport)}`)
      .then((result) => {
        if (result.success) {
    ...
    <>
      {
        canRender ?
          <div className="lg:col-span-4 border-l ps-8 most_active">
            ...
            <SharingExceedReport></SharingExceedReport>
          </div>
...
export default ExportExceedReport;
