...
const ReSubscriptions = () => {
  ...
  const columns = [
    {
      id: "resubscribeddate",
      name: "Resubscribed Date",
      cell: (row: CancelledSubscription) => ( row.createdAt ? formatDate(row.createdAt).toString() : "-" ),
      sortable: true,
    ...
    },
    {
      id: "IsActive",
      name: "Is Active",
      cell: (row: CancelledSubscription) => row.isActive ? "Yes" : "No",
      sortable: true,
      sortFunction: (a: CancelledSubscription, b: CancelledSubscription) =>  Number(a.isActive) - Number(b.isActive)
    },
    {
      id: "userName",
      name: "User Name",
  ...
  const getReSubscriptionDetails = (type = 0) => {
    setIsLoading(true);
    const postData = {
      type: industryType,
    ...
    if (type == 1) {
      authFileFetcherByPost(`${getEndpointUrl(ENDPOINTS.downloadReSubscriptionCsv)}`, postData).then((result) => {
        const link = document.createElement("a");
        link.href = window.URL.createObjectURL(result);
        link.download = "XDS_Re_Subscriptions.csv";
        link.click();
      }).catch((err) => {
        console.log(err);
      }).finally(() => {
      ...
      authPostdata(`${getEndpointUrl(ENDPOINTS.getReSubscriptionDetails)}`, postData).then((result) => {
  ...
  useEffect(() => {
    getReSubscriptionDetails();
  }, [industryType, timeLineVal, theTimeLine]);

  return (
    <>
      <div className="sm:flex sm:items-center sm:justify-between" id="ReSubscriptions">
        ...
        <div className="grid grid-cols-1 gap-3 lg:grid-cols-4 xl:grid-cols-4 2xl:grid-cols-4 md:grid-cols-2 sm:grid-cols-2 pt-3" >
          ...
          <div>
            <button
              ...
              type="button"
              onClick={(e) => { e.preventDefault(); getReSubscriptionDetails(1); }}
            >
              {" "}
              <svg
...
export default ReSubscriptions;