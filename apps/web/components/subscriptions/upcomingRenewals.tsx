  ...
  const columns = [
    {
      ...
      sortable: true,
      sortFunction: (a: Subscription, b: Subscription) =>  getRenewalDate(b.stripeExpireDate).getTime() - getRenewalDate(a.stripeExpireDate).getTime(),
    },
    {
      id: "userName",
      name: "User Name",
  ...
  const getSubscriptions = (type = 0) => {
    ...
    if(type == 1) {
      authFileFetcherByPost(`${getEndpointUrl(ENDPOINTS.downloadSubscriptionCsv)}`, postData).then((result) => {
        ...
        link.download = "XDS_Upcoming_Renewals.csv";
        link.click();
      }).catch((err) => {
        console.log(err);
      }).finally(() => {
    ...
    <>
      ...
      <div className="py-6 datatable_style">
        <DataTable
          ...
          pagination={true}
          defaultSortFieldId="renewalDate" // Specify the default sorted column (e.g., 'name')
