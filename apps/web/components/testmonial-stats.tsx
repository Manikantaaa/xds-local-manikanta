...
export interface AnnoucementStat {
  ...
  announcement: { company: {id: number, name: string, slug: string }, linkUrl?: string, title: string, createdAt: Date, isActive: boolean };
  companies: { name: string, clickCount: number, viewCount: number }[];
  clickCount: number;
  viewCount: number;
}

const TestmonialStats = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: (row: AnnoucementStat) => (
        <>{ row.announcement.linkUrl &&
            <div className="text-blue-300">
              <Link prefetch={false} href={row.announcement?.linkUrl &&
    ...
    // {
    //   id: "externalLink",
    //   name: "Link to Project Highlight",
    //   cell: (row: AnnoucementStat) => (
    //     <>
    //       {
    //         row.announcement.linkUrl ?
        ...
        <div>
          <button
            ...
            onClick={() => {
              ...
              setAnnouncementStatus(row.announcement.isActive);
              setOpenArchiveStatusModel(true);
            }}
          >
            {
              !row.announcement.isActive ?
                <Tooltip content="Show Testimonial" className="tier_tooltip_announcement">
                ...
                :
                <Tooltip content="Hide Testimonial" className="tier_tooltip_announcement">
                  <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 32 32" fill="none">
  ...
  const getAnnouncementDetails = async () => {
    setIsLoading(true);
    const result = await authFetcher(getEndpointUrl(ENDPOINTS.getTestmonialsStats(searchString)));
  ...
  const updateArchiveStatus = async () => {
    ...
    const response = await authPostdata(getEndpointUrl(ENDPOINTS.manageTestmonialStatViewCount), { ids: announcementId, type: "hide", toggle: announcementStatus }).catch((err) => {
      console.log(err);
    });
      ...
      const updatedAnnouncements = announcementStats.map(item =>
        item.announcementId === announcementId ? {
          ...
          announcement: {
            ...
            isActive: !item.announcement.isActive
          }
        } : item
      );
          ...
          <div className="pb-6">
              ...
              <div className="relative w-full">
                ...
                <input
                  ...
                  placeholder="Search by Company, Project Highlight"
                  required
                  value={searchString}
                />
              </div>
            ...
            <div className="space-y-6">
              <div className="">
                <p className="text-sm default_text_color font-normal leading-6">
                  {!announcementStatus ? 'You are about to show the Testimonial' : 'You are about to hide the Testimonial'}
                </p>
              </div>
            </div>
          </Modal.Body>
...
export default TestmonialStats;